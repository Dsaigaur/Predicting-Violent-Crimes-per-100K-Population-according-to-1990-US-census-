...
# Finally After Pre-processing the data has 2214 records and 113 columns
...
## 1. H0: The number of people who commit crime in age's 12 to 24 is less than compared to that of age's 12 to 29.
## A. H0 is rejected. As age's in 12 to 24 is more than that of age's 12 to 29.
## age12t29 -- 61233.32
## age12t24 -- 31997.53


sum(data$agePct12t21)

sum(data$agePct12t29)


## 2. H0: The total number of crimes are more in states like CA, TX and NJ comapared to any state.
## A. H0 is accepted. CA, FL, NJ have one of the highest number of crimes reported.
## CA -- 279
## MA -- 123
## TX -- 162
## NJ -- 211

aggregate(data.frame(count = data$state), list(value = data$state), length)


## 3. H0: The number of crimes that have took place include individuals from every race equally.
## A. H0 is rejected. The number of crimes commited are more in number for white and black race than that of any other.
...
sum(data$racepctblack)
sum(data$racePctWhite)
sum(data$racePctAsian)
sum(data$racePctHisp)
